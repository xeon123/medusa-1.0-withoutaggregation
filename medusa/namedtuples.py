...
def set_execution_params(job_name, currentqueuecapacity, hdfsbytesread, hdfsbyteswritten, maps, reduces, mem_load,
                         cpu_load, total_time):
    """
    ...
    """
    ExecutionParams = namedtuple("ExecutionParams",
                                 ["job_name", "currentqueuecapacity", "hdfsbytesread", "hdfsbyteswritten",
...
def set_job_params(job_name, current_queue_capacity,
    ...
    JobParams = namedtuple("JobParams",
                           ["job_name", "current_queue_capacity", "input_size", "output_size", "maps", "cpu_load",
                            "mem_load", "total_time"])
    job_params = JobParams(job_name, current_queue_capacity, input_size, output_size, maps, cpu_load, mem_load,
                           total_time)
...
def set_job_rank(cluster, predicted_time):
    """
    Set job predicted time

    :return (nametuple)
    """

    JobRank = namedtuple("JobRanking", ["cluster", "predicted_time"])
    job_rank = JobRank(cluster, predicted_time)
