...
def run_element(jobs, boolean_result):
    """
    ...
    digests_matrix = run_execution(
        faults_tolerate, jobs, boolean_result)
    mend = time.time()
    ...
    validation, digest_selected, failed_launch_command = run_verification(
        digests_matrix, faults_tolerate)
    vend = time.time()
    ...
    while not validation:
        ...
        digests_matrix = run_execution_faultmode(
            99, failed_launch_command, boolean_result, digests_matrix)
